// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'whole_hours_total_worked.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#adding-getters-and-methods-to-our-models');

WholeHoursTotalWorked _$WholeHoursTotalWorkedFromJson(
    Map<String, dynamic> json) {
  return _WholeHoursTotalWorked.fromJson(json);
}

/// @nodoc
mixin _$WholeHoursTotalWorked {
  num? get annual => throw _privateConstructorUsedError;
  @JsonKey(name: 'full_time')
  num? get fullTime => throw _privateConstructorUsedError;
  @JsonKey(name: 'half_time')
  num? get halfTime => throw _privateConstructorUsedError;
  @JsonKey(name: 'normal_hours')
  num? get normalHours => throw _privateConstructorUsedError;
  @JsonKey(name: 'other_days')
  num? get otherDays => throw _privateConstructorUsedError;
  @JsonKey(name: 'public_holidays')
  num? get publicHolidays => throw _privateConstructorUsedError;
  num? get sick => throw _privateConstructorUsedError;
  @JsonKey(name: 'total_hours_worked')
  num? get totalHoursWorked => throw _privateConstructorUsedError;

  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $WholeHoursTotalWorkedCopyWith<WholeHoursTotalWorked> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $WholeHoursTotalWorkedCopyWith<$Res> {
  factory $WholeHoursTotalWorkedCopyWith(WholeHoursTotalWorked value,
          $Res Function(WholeHoursTotalWorked) then) =
      _$WholeHoursTotalWorkedCopyWithImpl<$Res, WholeHoursTotalWorked>;
  @useResult
  $Res call(
      {num? annual,
      @JsonKey(name: 'full_time') num? fullTime,
      @JsonKey(name: 'half_time') num? halfTime,
      @JsonKey(name: 'normal_hours') num? normalHours,
      @JsonKey(name: 'other_days') num? otherDays,
      @JsonKey(name: 'public_holidays') num? publicHolidays,
      num? sick,
      @JsonKey(name: 'total_hours_worked') num? totalHoursWorked});
}

/// @nodoc
class _$WholeHoursTotalWorkedCopyWithImpl<$Res,
        $Val extends WholeHoursTotalWorked>
    implements $WholeHoursTotalWorkedCopyWith<$Res> {
  _$WholeHoursTotalWorkedCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? annual = freezed,
    Object? fullTime = freezed,
    Object? halfTime = freezed,
    Object? normalHours = freezed,
    Object? otherDays = freezed,
    Object? publicHolidays = freezed,
    Object? sick = freezed,
    Object? totalHoursWorked = freezed,
  }) {
    return _then(_value.copyWith(
      annual: freezed == annual
          ? _value.annual
          : annual // ignore: cast_nullable_to_non_nullable
              as num?,
      fullTime: freezed == fullTime
          ? _value.fullTime
          : fullTime // ignore: cast_nullable_to_non_nullable
              as num?,
      halfTime: freezed == halfTime
          ? _value.halfTime
          : halfTime // ignore: cast_nullable_to_non_nullable
              as num?,
      normalHours: freezed == normalHours
          ? _value.normalHours
          : normalHours // ignore: cast_nullable_to_non_nullable
              as num?,
      otherDays: freezed == otherDays
          ? _value.otherDays
          : otherDays // ignore: cast_nullable_to_non_nullable
              as num?,
      publicHolidays: freezed == publicHolidays
          ? _value.publicHolidays
          : publicHolidays // ignore: cast_nullable_to_non_nullable
              as num?,
      sick: freezed == sick
          ? _value.sick
          : sick // ignore: cast_nullable_to_non_nullable
              as num?,
      totalHoursWorked: freezed == totalHoursWorked
          ? _value.totalHoursWorked
          : totalHoursWorked // ignore: cast_nullable_to_non_nullable
              as num?,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$WholeHoursTotalWorkedImplCopyWith<$Res>
    implements $WholeHoursTotalWorkedCopyWith<$Res> {
  factory _$$WholeHoursTotalWorkedImplCopyWith(
          _$WholeHoursTotalWorkedImpl value,
          $Res Function(_$WholeHoursTotalWorkedImpl) then) =
      __$$WholeHoursTotalWorkedImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {num? annual,
      @JsonKey(name: 'full_time') num? fullTime,
      @JsonKey(name: 'half_time') num? halfTime,
      @JsonKey(name: 'normal_hours') num? normalHours,
      @JsonKey(name: 'other_days') num? otherDays,
      @JsonKey(name: 'public_holidays') num? publicHolidays,
      num? sick,
      @JsonKey(name: 'total_hours_worked') num? totalHoursWorked});
}

/// @nodoc
class __$$WholeHoursTotalWorkedImplCopyWithImpl<$Res>
    extends _$WholeHoursTotalWorkedCopyWithImpl<$Res,
        _$WholeHoursTotalWorkedImpl>
    implements _$$WholeHoursTotalWorkedImplCopyWith<$Res> {
  __$$WholeHoursTotalWorkedImplCopyWithImpl(_$WholeHoursTotalWorkedImpl _value,
      $Res Function(_$WholeHoursTotalWorkedImpl) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? annual = freezed,
    Object? fullTime = freezed,
    Object? halfTime = freezed,
    Object? normalHours = freezed,
    Object? otherDays = freezed,
    Object? publicHolidays = freezed,
    Object? sick = freezed,
    Object? totalHoursWorked = freezed,
  }) {
    return _then(_$WholeHoursTotalWorkedImpl(
      annual: freezed == annual
          ? _value.annual
          : annual // ignore: cast_nullable_to_non_nullable
              as num?,
      fullTime: freezed == fullTime
          ? _value.fullTime
          : fullTime // ignore: cast_nullable_to_non_nullable
              as num?,
      halfTime: freezed == halfTime
          ? _value.halfTime
          : halfTime // ignore: cast_nullable_to_non_nullable
              as num?,
      normalHours: freezed == normalHours
          ? _value.normalHours
          : normalHours // ignore: cast_nullable_to_non_nullable
              as num?,
      otherDays: freezed == otherDays
          ? _value.otherDays
          : otherDays // ignore: cast_nullable_to_non_nullable
              as num?,
      publicHolidays: freezed == publicHolidays
          ? _value.publicHolidays
          : publicHolidays // ignore: cast_nullable_to_non_nullable
              as num?,
      sick: freezed == sick
          ? _value.sick
          : sick // ignore: cast_nullable_to_non_nullable
              as num?,
      totalHoursWorked: freezed == totalHoursWorked
          ? _value.totalHoursWorked
          : totalHoursWorked // ignore: cast_nullable_to_non_nullable
              as num?,
    ));
  }
}

/// @nodoc
@JsonSerializable()
class _$WholeHoursTotalWorkedImpl implements _WholeHoursTotalWorked {
  _$WholeHoursTotalWorkedImpl(
      {this.annual,
      @JsonKey(name: 'full_time') this.fullTime,
      @JsonKey(name: 'half_time') this.halfTime,
      @JsonKey(name: 'normal_hours') this.normalHours,
      @JsonKey(name: 'other_days') this.otherDays,
      @JsonKey(name: 'public_holidays') this.publicHolidays,
      this.sick,
      @JsonKey(name: 'total_hours_worked') this.totalHoursWorked});

  factory _$WholeHoursTotalWorkedImpl.fromJson(Map<String, dynamic> json) =>
      _$$WholeHoursTotalWorkedImplFromJson(json);

  @override
  final num? annual;
  @override
  @JsonKey(name: 'full_time')
  final num? fullTime;
  @override
  @JsonKey(name: 'half_time')
  final num? halfTime;
  @override
  @JsonKey(name: 'normal_hours')
  final num? normalHours;
  @override
  @JsonKey(name: 'other_days')
  final num? otherDays;
  @override
  @JsonKey(name: 'public_holidays')
  final num? publicHolidays;
  @override
  final num? sick;
  @override
  @JsonKey(name: 'total_hours_worked')
  final num? totalHoursWorked;

  @override
  String toString() {
    return 'WholeHoursTotalWorked(annual: $annual, fullTime: $fullTime, halfTime: $halfTime, normalHours: $normalHours, otherDays: $otherDays, publicHolidays: $publicHolidays, sick: $sick, totalHoursWorked: $totalHoursWorked)';
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$WholeHoursTotalWorkedImpl &&
            (identical(other.annual, annual) || other.annual == annual) &&
            (identical(other.fullTime, fullTime) ||
                other.fullTime == fullTime) &&
            (identical(other.halfTime, halfTime) ||
                other.halfTime == halfTime) &&
            (identical(other.normalHours, normalHours) ||
                other.normalHours == normalHours) &&
            (identical(other.otherDays, otherDays) ||
                other.otherDays == otherDays) &&
            (identical(other.publicHolidays, publicHolidays) ||
                other.publicHolidays == publicHolidays) &&
            (identical(other.sick, sick) || other.sick == sick) &&
            (identical(other.totalHoursWorked, totalHoursWorked) ||
                other.totalHoursWorked == totalHoursWorked));
  }

  @JsonKey(ignore: true)
  @override
  int get hashCode => Object.hash(runtimeType, annual, fullTime, halfTime,
      normalHours, otherDays, publicHolidays, sick, totalHoursWorked);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$WholeHoursTotalWorkedImplCopyWith<_$WholeHoursTotalWorkedImpl>
      get copyWith => __$$WholeHoursTotalWorkedImplCopyWithImpl<
          _$WholeHoursTotalWorkedImpl>(this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$WholeHoursTotalWorkedImplToJson(
      this,
    );
  }
}

abstract class _WholeHoursTotalWorked implements WholeHoursTotalWorked {
  factory _WholeHoursTotalWorked(
          {final num? annual,
          @JsonKey(name: 'full_time') final num? fullTime,
          @JsonKey(name: 'half_time') final num? halfTime,
          @JsonKey(name: 'normal_hours') final num? normalHours,
          @JsonKey(name: 'other_days') final num? otherDays,
          @JsonKey(name: 'public_holidays') final num? publicHolidays,
          final num? sick,
          @JsonKey(name: 'total_hours_worked') final num? totalHoursWorked}) =
      _$WholeHoursTotalWorkedImpl;

  factory _WholeHoursTotalWorked.fromJson(Map<String, dynamic> json) =
      _$WholeHoursTotalWorkedImpl.fromJson;

  @override
  num? get annual;
  @override
  @JsonKey(name: 'full_time')
  num? get fullTime;
  @override
  @JsonKey(name: 'half_time')
  num? get halfTime;
  @override
  @JsonKey(name: 'normal_hours')
  num? get normalHours;
  @override
  @JsonKey(name: 'other_days')
  num? get otherDays;
  @override
  @JsonKey(name: 'public_holidays')
  num? get publicHolidays;
  @override
  num? get sick;
  @override
  @JsonKey(name: 'total_hours_worked')
  num? get totalHoursWorked;
  @override
  @JsonKey(ignore: true)
  _$$WholeHoursTotalWorkedImplCopyWith<_$WholeHoursTotalWorkedImpl>
      get copyWith => throw _privateConstructorUsedError;
}
